...
public class TicketOperationService {
	 ...
	 * 
	 * @param id The ID of the user requesting the ticket information. If the user
	 *           is an admin (ID = 1), all non-deleted tickets will be returned. For
	 *           regular users, only tickets that they have created or are assigned
	 *           to will be returned.
	 * @return A list of tickets based on the user's access rights (admin or regular
	 *         user).
	 */
	 ...
	 * 
	 * @param ticketDTO A Data Transfer Object containing the necessary ticket
	 *                  information such as title, description, priority, status,
	 *                  etc.
	 * @throws IllegalArgumentException if any required fields createdBy or
	 *                                  assignedTo are missing in the DTO.
	 * @throws RuntimeException         if any referenced entities priority, status,
	 *                                  user are not found.
	 */
	...
	/**
	 * Updates an existing ticket based on the provided ticket ID and new ticket
	 * data from TicketDTO.
	 * 
	...
	/**
	 * Soft deletes a ticket by setting its deleted flag to true and assigning a
	 * deletion date.
	 * 
	 * @param id        The ID of the ticket to be deleted.
	 * @param ticketDTO A DTO containing any necessary details for deleting the
	 *                  ticket (though only deletion status is used here).
